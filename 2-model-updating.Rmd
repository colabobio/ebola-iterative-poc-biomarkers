...
sel_thres <- 0.5    # Minimu fraction of models in which a variable appears as a positive predictor in order to be selected
...
```{r}
cov12 <- cor(data12[,sel12$Variable], use='pairwise') > max_cor
cov12
```
...
```{r}
cov34 <- cor(data34[,sel34$Variable], use='pairwise') > max_cor
cov34
```
...
```{r}
cov56 <- cor(data56[,sel56$Variable], use='pairwise') > max_cor
cov56
```
...
```{r}
# Apply threshold
sel78 <- varf78 %>% filter(Freq > sel_thres)
sel78
```

```{r}
cov78 <- cor(data78[,sel78$Variable], use='pairwise') > max_cor
cov78
```
